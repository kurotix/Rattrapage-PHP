...
<body>
		...
		<?php 
				...
				// afficher le cadeau d'une liste
                $result=mysqli_query($co,"SELECT NomCadeau FROM cadeau natural join contient WHERE IdListe= (SELECT IdListe FROM liste  WHERE NomListe= '$NomListe')  ") or die("erreur de requete"); 


                     
                 echo '<table width="15%"><br><br> 
                <tr><th>Liste :</th></tr>';  
				// affichage des cadeaux contenu dans liste

                while($data=mysqli_fetch_assoc($result)){ 
                    echo '<tr><td>'.$data['NomCadeau'].'</td>/tr>';
                }
				// tableau associatif => résultat de la requête dont le nom résulte stocker dans une variable data
